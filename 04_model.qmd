...
Before providing a detailed outline of the model (see below), we briefly describe the basic intuition underlying the agent-based model we use to simulate the household decision to adopt green technologies. At present, the model only depicts the adoption of photovoltaic (PV) systems, but the same theoretical framework could also be applied to other technologies such as heat pumps, insulation, or AC systems.

The adoption decision in the model is based on a simple expected utility framework; in every period $t = 1,...,T$ each household $h = 1,...,N$ who does not already own PV faces the binary decision whether or not to purchase the single type of PV system available in the model. The expected utility from owning PV is a function of the profit $h$ expects to derive from the ownership of PV over the lifetime of the panel, as well as a social influence based component and one based on the environmental attitude of $h$. The expected profit from owning PV (stemming from an expected reduction in outlays for electricity) is in turn a function of the future electricity price expected by $h$.

Households receive income, consume and pay electricity cost, resulting in changes in their stocks of liquid assets; these liquid assets can in turn be used to purchase a solar panel. Households with insufficient liquid assets may also request a loan to install PV from a stylised banking sector, but may be credit-rationed if the bank is unwilling to lend them as much as they require.

# Model description

sHouseholds' initial characteristics are set using SOEP data alongside other data sources. Parameter values are set following empirical evidence as far as possible.
...
$$
R^h_t =R^h_{t-1}+\gamma_h\left(x_t x_t'-R^h_{t-1}\right)\\ 
n^h_t =n^h_{t-1}+\gamma_h\left(R^h_t\right)^{-1}x_t(y_t-x_t' n^h_{t-1})
$$
